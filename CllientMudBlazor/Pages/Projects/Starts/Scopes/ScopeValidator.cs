...
{
    public class ScopeValidator   :AbstractValidator<ScopeDto>
    {
        private IHttpServices Service;
        public ScopeValidator(IHttpServices service)
        {
            Service = service;
        ...
        async Task<bool> ReviewIfNameExist(ScopeDto request, string name, CancellationToken cancellationToken)
        {
            ValidateScopeName validate = new()
            {
                Name = request.Name,
                Id = request.Id,
